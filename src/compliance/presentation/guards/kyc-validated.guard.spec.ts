...
/**
 * Le garde demande à `InvestorComplianceProfile.peutOperer()` et traduit un
...
const makeGuard = (peutOperer: boolean) => {
  const profils = {
    findByInvestorId: jest
      .fn()
      .mockResolvedValue({ peutOperer: () => peutOperer }),
  };
...
describe('KycValidatedGuard', () => {
  it('laisse passer un titulaire apte', async () => {
    ...
    expect(profils.findByInvestorId).toHaveBeenCalledWith(42);
  });
  ...
  it('refuse sans utilisateur authentifié (403 générique, sans code KYC)', async () => {
    ...
    expect(profils.findByInvestorId).not.toHaveBeenCalled();
  });
});
