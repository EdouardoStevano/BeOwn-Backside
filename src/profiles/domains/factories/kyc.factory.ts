import { KycNiveau } from 'src/profiles/domains/enums/kyc-status.enum';
import { ChampProfilInvalideError } from 'src/profiles/domains/errors';
import { Kyc } from 'src/profiles/domains/kyc';
import { DecisionKyc } from 'src/profiles/domains/value-objects/decision-kyc.vo';
import { eprouverUtilisateurId } from 'src/profiles/domains/value-objects/identifiant-utilisateur';
import { Libelle } from 'src/profiles/domains/value-objects/libelle.vo';
...
export class KycFactory {
  static creer(props: CreerKycProps): Kyc {
    return new Kyc({
      ...
      scoreRisque: null,
      fournisseur: eprouverFournisseur(
...
/**
 * Le fournisseur est la seule donnée que l'appelant fournit en propre, et la
 * colonne est `NOT NULL` : une chaîne vide y passerait sans bruit et rendrait
 * le dossier impossible à rattacher à un prestataire — donc à relancer.
 */
function eprouverFournisseur(raw: string): string {
  const fournisseur = Libelle.of(
    raw,
