...
import { CacheManagerService } from '../../domains/ports/cache-manager.port';
import { CACHE_MANAGER, type Cache } from '@nestjs/cache-manager';
import { AuthSession, EmailTokenPurpose } from '../../domains/ports/token.port';
import jwtConfig from '../config/jwt.config';
import { type ConfigType } from '@nestjs/config';

export class RedisCacheAdapter implements CacheManagerService {
  constructor(
  ...
  async insertOAuthCode(code: string, session: AuthSession): Promise<void> {
    await this.cacheManager.set(`oauth-code:${code}`, session, 30_000);
  }
