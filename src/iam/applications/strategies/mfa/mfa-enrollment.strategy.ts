 ...
 * (§4 Open/Closed).
 */
export interface MfaEnrollmentStrategy {
  ...
  start(request: MfaEnrollmentRequest): Promise<MfaEnrollmentChallenge>;
  /** Valide le défi et fait de la méthode l'unique méthode active du canal. */
  confirm(confirmation: MfaEnrollmentConfirmation): Promise<void>;
  /**
...
/** Token du tableau de stratégies injecté dans `EnrollMfaUseCase`. */
export const MFA_ENROLLMENT_STRATEGIES = Symbol('MFA_ENROLLMENT_STRATEGIES');
