...
import { KycError, KycErrorKind } from 'src/kyc/domains/errors';

/** Seule table qui connaît HTTP : le domaine ignore ces statuts. */
const STATUS_BY_KIND: Record<KycErrorKind, HttpStatus> = {
  [KycErrorKind.FORBIDDEN]: HttpStatus.FORBIDDEN,
  [KycErrorKind.NOT_FOUND]: HttpStatus.NOT_FOUND,
  [KycErrorKind.CONFLICT]: HttpStatus.CONFLICT,
  [KycErrorKind.INVALID_INPUT]: HttpStatus.BAD_REQUEST,
  [KycErrorKind.UNEXPECTED]: HttpStatus.INTERNAL_SERVER_ERROR,
};
 ...
 *
 * `@Catch(KycError)` : tout le reste — `HttpException` levées par les
 ...
 */
@Catch(KycError)
export class KycErrorFilter implements ExceptionFilter<KycError> {
  private readonly logger = new Logger(KycErrorFilter.name);

  catch(error: KycError, host: ArgumentsHost): void {
    ...
    if (error.kind === KycErrorKind.UNEXPECTED) {
