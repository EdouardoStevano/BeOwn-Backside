...
export class ContractGeneratorService {
  async generateBulletin(data: ContractData): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      // Brand
      doc
        .fillColor('white').fontSize(24).font('Helvetica-Bold')
        .text('BeOwn', M, 30);
      doc
        .fillColor('#CBD5E1').fontSize(9).font('Helvetica')
        .text("Plateforme d'investissement immobilier fractionné", M, 60);

      // Ref + date (right)
      doc
        .fillColor(AMBER).fontSize(10).font('Helvetica-Bold')
        .text(`Réf. ${ref}`, W - M - 200, 32, { width: 200, align: 'right' });
      doc
        .fillColor('#CBD5E1').fontSize(8).font('Helvetica')
        .text(`Émis le ${date} à ${time}`, W - M - 200, 50, { width: 200, align: 'right' });

      // ── Title block ──
      let y = 132;
      doc
        .fillColor(NAVY).fontSize(22).font('Helvetica-Bold')
        .text('Bulletin de Souscription', M, y, { width: innerW, align: 'center' });
      y += 30;
      doc
      ...
      doc
        .fillColor(AMBER).fontSize(8).font('Helvetica-Bold')
        .text('MONTANT TOTAL SOUSCRIT', M + 20, y + 14);
      doc
        .fillColor('white').fontSize(22).font('Helvetica-Bold')
        .text(
          `${Number(data.investment.montant).toLocaleString('fr-FR')} XOF`,
          M + 20, y + 28,
        );
      ...
      const section = (title: string) => {
        ...
        doc
          .fillColor(NAVY).fontSize(9.5).font('Helvetica-Bold')
      ...
      const row = (label: string, value: string) => {
        ...
        doc.moveTo(M + 12, startY + 16).lineTo(W - M - 12, startY + 16)
      ...
      row('Nom complet', `${data.investorFirstname} ${data.investorLastname}`.trim() || '—');
      ...
      row(
        'Valeur par fraction',
        `${Number(data.investment.valeurTitre ?? 0).toLocaleString('fr-FR')} XOF`,
      );
      row(
        'Montant total',
        `${Number(data.investment.montant).toLocaleString('fr-FR')} XOF`,
      ...
      // ── Legal notice ──
      doc.moveTo(M, y).lineTo(W - M, y).strokeColor(BORDER).lineWidth(0.8).stroke();
      y += 10;
      doc
        .fillColor(FAINT).fontSize(7.5).font('Helvetica')
        .text(
          ...
          M, y, { width: innerW, align: 'justify', lineGap: 2 },
        );
      ...
      doc
        .fillColor(FAINT).fontSize(7.5).font('Helvetica')
        .text(
          'BeOwn SAS — contact@beown.com — www.beown.com',
          0, footerY - 6, { width: W, align: 'center' },
        );
      doc
        .fillColor(FAINT).fontSize(7).font('Helvetica')
        .text(
  ...
  async generateContratSouscription(data: ContractData): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      doc.moveDown(4);
      doc.fillColor(primary).fontSize(18).font('Helvetica-Bold').text('Contrat de Souscription', { align: 'center' });
      doc.fontSize(11).font('Helvetica').fillColor('#64748B').text(`Investissement immobilier — ${data.projectTitle}`, { align: 'center' });
      doc.moveDown(1.5);
      ...
      const section = (title: string) => {
        ...
        doc.fillColor(primary).fontSize(10).font('Helvetica-Bold').text(title, 58, doc.y - 17);
        doc.moveDown(0.3);
      };
      const row = (label: string, value: string) => {
        const y = doc.y;
      ...
      doc.fillColor(accent).fontSize(11).font('Helvetica-Bold').text('MONTANT TOTAL', 58, doc.y - 30, { width: 200 });
      doc.fillColor('white').fontSize(16).font('Helvetica-Bold').text(`${Number(data.investment.montant).toLocaleString('fr-FR')} XOF`, 280, doc.y - 32);
      doc.moveDown(2);

      doc.moveTo(50, doc.y).lineTo(doc.page.width - 50, doc.y).strokeColor('#E2E8F0').lineWidth(1).stroke();
      doc.moveDown(1);
      doc.fillColor(primary).fontSize(10).font('Helvetica-Bold').text('SIGNATURE DU SOUSCRIPTEUR', 50);
      doc.moveDown(0.5);
      doc.fillColor('#64748B').fontSize(8).font('Helvetica')
        .text('En signant ce contrat, le souscripteur reconnaît avoir pris connaissance des conditions d\'investissement et accepte les termes du présent contrat de souscription.', 50, doc.y, { width: doc.page.width - 100 });
      doc.moveDown(1.5);
      doc.rect(50, doc.y, 200, 60).strokeColor('#CBD5E1').lineWidth(1).stroke();
      doc.fillColor('#94A3B8').fontSize(8).text('Signature', 58, doc.y + 4);
      doc.moveDown(2.5);

      doc.fillColor('#94A3B8').fontSize(7.5).font('Helvetica')
        .text('Ce contrat de souscription est établi conformément à la réglementation PSFP. Investir comporte des risques incluant la perte partielle ou totale du capital. BeOwn est enregistrée en tant que Prestataire de Services de Financement Participatif.',
  ...
  async generateContratRachat(data: ContratRachatData): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      doc.fontSize(11).font('Helvetica').fillColor('#64748B')
        .text(`Cession de fractions — ${data.projectTitle}`, { align: 'center' });
      doc.moveDown(1.5);
      ...
      row('Prix par fraction', `${Number(data.prixUnitaire).toLocaleString('fr-FR')} XOF`);
      ...
      doc.fillColor('white').fontSize(16).font('Helvetica-Bold')
        .text(`${Number(data.montantTotal).toLocaleString('fr-FR')} XOF`, 280, doc.y - 32);
      doc.moveDown(2);

      // ── Zone de signature ────────────────────────────────────────────────────
      doc.moveTo(50, doc.y).lineTo(doc.page.width - 50, doc.y).strokeColor('#E2E8F0').lineWidth(1).stroke();
      doc.moveDown(1);
      doc.fillColor(primary).fontSize(10).font('Helvetica-Bold').text('SIGNATURE DE L\'ACHETEUR', 50);
      doc.moveDown(0.5);
      doc.fillColor('#64748B').fontSize(8).font('Helvetica')
        .text('En signant ce document, l\'acheteur reconnaît avoir pris connaissance des conditions de cession et accepte les termes du présent contrat.', 50, doc.y, { width: doc.page.width - 100 });
      doc.moveDown(1.5);
      doc.rect(50, doc.y, 200, 60).strokeColor('#CBD5E1').lineWidth(1).stroke();
      doc.fillColor('#94A3B8').fontSize(8).text('Signature', 58, doc.y + 4);
      doc.moveDown(2);
