...
import { UserEntity, UserRole, UserStatus } from '../infrastructure/persistences/entities/user.entity';
import { NotificationService } from 'src/notifications/applications/notification.service';
import { NotificationEntity, NotificationType } from 'src/notifications/infrastructure/persistences/entities/notification.entity';
...
export class InvestorInactivityCronService {
  ...
  async flagInactiveInvestors(): Promise<void> {
    ...
    const candidates = await this.userRepo.find({
      where: [
        { role: UserRole.INVESTISSEUR, status: UserStatus.ACTIF, lastLoginAt: LessThan(cutoff) },
        { role: UserRole.INVESTISSEUR, status: UserStatus.ACTIF, lastLoginAt: IsNull(), createdAt: LessThan(cutoff) },
      ],
    });
    ...
    for (const u of candidates) {
      ...
      const recent = await this.notifRepo
        ...
        .where('n.type = :type', { type: NotificationType.INVESTISSEUR_INACTIF })
      ...
      await this.notifications.pushToAdmins({
        ...
        message: `${u.firstname ?? ''} ${u.lastname ?? ''}`.trim() +
    ...
    this.logger.log(`CRON inactivité: ${notified} investisseur(s) signalé(s) sur ${candidates.length} candidat(s)`);
  }
}
