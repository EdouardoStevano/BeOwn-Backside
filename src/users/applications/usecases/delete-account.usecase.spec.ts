...
describe('DeleteAccountUseCase', () => {
  ...
  it('cible déjà SUPPRIME → no-op idempotent, sans email ni notif ni event', async () => {
    userRepo.findOne.mockResolvedValue({ ...baseUser(), status: UserStatus.SUPPRIME });
  ...
  it('solde > 0 AVEC IBAN → retrait ATOMIQUE (tx + débit dans la même transaction) + 409 withdrawalCreated:true', async () => {
    ...
    await expect(useCase.execute(USER_ID, SELF)).rejects.toMatchObject({
      response: { blockers: [{ code: 'WALLET_BALANCE', withdrawalCreated: true }] },
    });

    // Transaction utilisée ; wallet lu SOUS VERROU pessimiste.
    expect(dataSource.transaction).toHaveBeenCalledTimes(1);
    const lockCall = manager.findOne.mock.calls.find((c: any) => c[0] === WalletEntity);
    ...
    const created = manager.create.mock.calls.find((c: any) => c[0] === TransactionEntity)[1];
    ...
    const walletSave = manager.save.mock.calls.find((c: any) => c[0] === WalletEntity);
  ...
  it('deux DELETE concurrents : collision de clé d\'idempotence (23505) → un seul retrait, withdrawalCreated:true', async () => {
    ...
    const dup = new QueryFailedError('insert', undefined, new Error('duplicate key') as any);
  ...
  it('échec du débit dans la transaction → rollback (l\'erreur remonte, compte NON supprimé)', async () => {
    ...
    await expect(useCase.execute(USER_ID, SELF)).rejects.toThrow('db down au débit');
    expect(userRepo.save).not.toHaveBeenCalled();
    expect(notifications.pushToAdmins).not.toHaveBeenCalled();
  });
  ...
  it('interdit la suppression d\'un compte détenant users:delete (autre super_admin) → 400', async () => {
    userRepo.findOne.mockResolvedValue({ ...baseUser(), userId: 2, role: 'super_admin' });
    await expect(useCase.execute(2, ADMIN)).rejects.toBeInstanceOf(BadRequestException);
    expect(userRepo.save).not.toHaveBeenCalled();
  });
