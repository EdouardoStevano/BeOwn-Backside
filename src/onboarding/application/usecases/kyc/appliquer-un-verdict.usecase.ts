...
import type { EvenementDeVerification } from 'src/onboarding/application/acl/evenement-identite.translator';
import { AnnoncesKycService } from 'src/onboarding/application/services/annonces-kyc.service';
import { ArchivageRapportKycService } from 'src/onboarding/application/services/archivage-rapport-kyc.service';
 ...
 * `DossierDEntreeEnRelation.accueillirVerdict`.
 */
@Injectable()
export class AppliquerUnVerdictUseCase {
  ...
  async execute(fait: EvenementDeVerification): Promise<IssueDuVerdict> {
    const profil = await this.profils.parTitulaire(fait.utilisateurId);
  ...
  private async appliquer(
    ...
    profil: DossierDEntreeEnRelation,
