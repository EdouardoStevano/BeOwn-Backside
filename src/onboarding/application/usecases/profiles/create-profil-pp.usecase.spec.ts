...
function monter(
  ...
  // Le port du contexte voisin sert le classement publié à côté du dossier. Il
  // est joué par l'agrégat réel, projeté comme le fait la Query : un profil qui
  // vient de naître n'a pas de questionnaire, le repli est donc celui du
  // domaine de l'adéquation, et non une valeur figée dans ce test.
  const evaluationVierge = new EvaluationDAdequation({
    investorId: 42,
    adequacy: null,
  });
  const profilsConformite = {
    duTitulaire: jest.fn().mockResolvedValue({
      classement: evaluationVierge.classement.toSnapshot(),
      etapeSuivante: evaluationVierge.etapeSuivanteDuQuestionnaire(),
      etapesRepondues: evaluationVierge.etapesReponduesDuQuestionnaire(),
    }),
  };
  ...
  const useCase = new CreateProfilPPUseCase(
    ...
    profilsConformite as unknown as AvancementDuQuestionnaireQuery,
