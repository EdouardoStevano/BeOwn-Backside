...
import type { AvancementDuQuestionnaireQuery } from 'src/adequacy/application/ports/avancement-du-questionnaire.query';
...
import type { ProfilPPRepository } from 'src/onboarding/domain/repositories/profil-pp.repository';
...
function monter(
  ...
  const useCase = new GetOnboardingStatusUseCase(
    ...
    // Deux contextes depuis la scission : l'entrée en relation porte le dossier
    // de vérification, l'adéquation porte le questionnaire et le classement.
    // Cette projection est le seul endroit qui les lise ensemble.
    {
    ...
    } as unknown as DossierDEntreeEnRelationRepository,
    avancementDe(
...
/**
 * Le port d'avancement, joué par l'agrégat réel.
 *
 * L'évaluation est montée pour de bon puis projetée comme le fait
 * `AvancementDuQuestionnaireTypeOrmQuery` : quelle étape vient reste décidé par
 * le domaine de l'adéquation, et non par un double de test qui figerait la
 * réponse. Ce que ces tests vérifient, c'est ce que l'entrée en relation en
 * publie.
 */
function avancementDe(
  evaluation: EvaluationDAdequation,
): AvancementDuQuestionnaireQuery {
  return {
    duTitulaire: jest.fn().mockResolvedValue({
...
describe('GetOnboardingStatusUseCase', () => {
  ...
  it("survit à un dossier illisible plutôt que de refuser la page d'accueil", async () => {
    const useCase = new GetOnboardingStatusUseCase(
      ...
      avancementDe(EvaluationDAdequation.vierge(UTILISATEUR)),
    );
