...
/**
 * Dossier de l'investisseur : profil personne physique, profils personne
 * morale, pièces justificatives, bénéficiaires effectifs.
 *
 ...
 *
 * **Le questionnaire d'adéquation a quitté ce module** avec son contexte : voir
 * `AdequacyModule`. §3.3 invoquait RG-KYC-13 — « la catégorisation PSFP dérive
 * du questionnaire » — pour refuser de les séparer, au motif que cela créerait
 * une dépendance cyclique. Cette dépendance n'existe plus : le classement a
 * quitté `ProfilPP` pour `EvaluationDAdequation`, et la dérivation est à sens
 * unique, entièrement contenue dans cette racine-là. Aucun des deux contextes ne
 * lit un champ de l'autre — `peutOperer()` ne consulte jamais le classement, le
 * questionnaire ne consulte jamais le KYC.
 *
 * Deux lectures traversent encore la frontière, et c'est tout : le classement
 * opposable, pour composer l'éligibilité publiée à l'aval, et l'avancement du
 * questionnaire, parce que `GetOnboardingStatusUseCase` raconte le parcours
 * d'un seul tenant. Les deux passent par un port de l'adéquation et rendent des
 * primitives (§11).
 */
@Module({
  imports: [
    ...
    // `DOSSIER_ENTREE_EN_RELATION_REPOSITORY` — la racine du contexte — et
    // `PROFIL_CONFORMITE_QUERY`. Ce module réexporte aussi
    // `AdequacyInfrastructureModule` pour les deux lectures du questionnaire.
